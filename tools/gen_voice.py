...
Needs an ElevenLabs key in .env (ELEVENLABS_API_KEY, plus optional spares
ELEVENLABS_API_KEY_2..9 it falls through to on quota — see tools/eleven_keys.py).
...
from eleven_keys import KeyPool, QuotaExhausted, is_quota_error  # noqa: E402
    ...
    try:
        ...
        if e.code in (400, 404, 422):  # model may not support timestamps — plain fallback
            ...
            try:
                ...
                d2 = e2.read().decode()[:500]
                if is_quota_error(e2):  # spent key — let the pool fall through to the next one
                    raise QuotaExhausted(e2.code, d2)
                sys.exit(f"ElevenLabs TTS failed ({e2.code}) for: {text!r}\n{d2}")
        elif is_quota_error(e):  # spent key — let the pool fall through to the next one
            raise QuotaExhausted(e.code, detail)
        else:
...
def main():
    ...
    pool = KeyPool(required=not args.dry_run)  # primary + spares, auto-failover on quota
    ...
    for i, line in enumerate(vo):
        ...
        if args.force or not os.path.exists(raw) or not os.path.exists(raw + ".words.json"):
            ...
            pool.call(tts_line, args.voice, args.model, tts_text, prev_text, next_text, raw)
