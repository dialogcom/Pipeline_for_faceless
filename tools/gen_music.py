...
Needs an ElevenLabs key in .env (ELEVENLABS_API_KEY + optional spares _2.._9,
auto-failover on quota — see tools/eleven_keys.py). ffmpeg/ffprobe on PATH.
"""
import json
import os
import re
...
from eleven_keys import KeyPool  # noqa: E402
...
def main():
    ...
    pool = KeyPool(required=False)  # primary + spares, auto-failover on quota
    ...
    if dry:
        ...
        return
    if todo and not pool:
        sys.exit("no ElevenLabs key in .env — cannot generate.\n"
                 "  add one: python tools/eleven_keys.py --add   (or use --dry-run)")

    for s, rel, path, length_ms in todo:
        print(f"\n-> {s['id']}  ({length_ms/1000:.0f}s)")
        try:
            audio = pool.call(generate, s["prompt"], length_ms, model, force_instrumental, output_format)
