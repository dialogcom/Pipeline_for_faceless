...
13. **short-13 · Copywriting (RU)** — "Первый экран решает всё" ✅ — an ad-hoc user-requested
   topic (the landing-page headline+subheadline rule), inserted ahead of the planned 8-new-engines
   slate below rather than displacing it — same pattern as short-7 taking a slot. Reuses
   `lib/browser.tsx`, no new niche lib. First **Russian-language** short in the channel.
   - **Shipped twice.** First pass: no ElevenLabs key in `.env`, so it went out silent/
     captions-only per the user's own fallback instruction, `beats.json.vo[]` holding estimated
     (~2.7 words/sec) timings. The user then added a key and asked for a real voice pass.
   - **The estimate broke, and the tool's own OVERFLOW warning caught it before it shipped.**
     Real ElevenLabs Russian delivery ran slower than 2.7 words/sec on two reveal lines (a
     colon-pause sentence, a compound word) — even at `gen_voice.py`'s hard 1.3x tempo cap, both
     clips still overran their window by ~0.3s, which would have played two lines' audio at
     once. Fixed by widening those two lines' windows in `beats.json` (shifting every later cue
     start) and re-running — same text hash, so the raw ElevenLabs clips were cached and the fix
     cost **zero extra billing**, only a local ffmpeg re-fit. Runtime grew 36s→38s. Every
     hand-placed visual cue (marker sweeps, ring pulse, quiz-card exit, twist/loop) was then
     re-derived from the real per-word alignment, not the original guess — same discipline as
     short-8/short-10's "retime against the real voice" lesson, now proven on a non-English
     script too. See `shorts/short-13-landing/script.md` for the full before/after cue table.
   - **Cyrillic fonts, and it's a real gap in the shared kit.** `FONT_DISPLAY` (Space Grotesk)
     has ZERO cyrillic glyphs in Google Fonts (verified: grepped the bundled font metadata,
     0 hits vs Inter's 40). Added `FONT_BODY_CYR` to `fonts.ts` (Inter, latin+cyrillic, additive —
     `FONT_BODY` itself untouched). But `Captions`/`Kicker`/`BigTitle`/`Stamp`/`PauseCard` in
     `lib/shorts.tsx` all hardcode `FONT_DISPLAY` (captions) or `FONT_BODY` (latin-only), so
     `Short13Landing.tsx` doesn't reuse them for any cyrillic text — it has small local clones
     built on `FONT_BODY_CYR` instead. **Any future non-English short needs the same treatment**
     until/unless the shared kit grows a cyrillic-safe display font.
   - **Render environment bug, fixed at the tool level, not worked around.** `render-all.mjs`'s
     `renderMedia()` SIGABRTs on this machine (macOS 13.2.1): the bundled
     `@remotion/compositor-darwin-arm64` ffmpeg links `libavdevice` against
     `_AVCaptureDeviceTypeContinuityCamera`, an AVFoundation symbol that only exists on macOS 15+.
     It crashes both in the silent-audio-track step AND the final video stitch — `muted: true`
     alone doesn't dodge it. Fix: the opaque/h264 render path now uses `renderFrames` (pure
     headless-Chrome screenshots, no ffmpeg) into a temp dir, then stitches with the **system**
     ffmpeg (already a required tool per CLAUDE.md) instead of the broken bundled binary. This
     was blocking **every** shot's render on this machine, not just this one — the transparent/
     ProRes path still uses `renderMedia` (untested, unused by any shot today) and would need the
     same fix if it's ever hit.
   - Structure reuses the short-10/11/12 "rewind earns the loop" pattern: frame 0 is the
     finished good hero, SETUP dissolves it to a generic bad one, REVEAL rebuilds headline then
     subheadline+CTA on two independent crossfade tracks (so each resolves under its own VO
     line), TWIST calls back to the hook's "3 seconds", LOOP needs no motion at all because the
     reveal already left the canvas at frame 0's state.
...
## short-16..22 · "УКА" reflection-question series (2026-08-04)

Ad-hoc user-requested series, outside the entertainment-niche ranking above: 7 silent, text-only
Reels/Shorts (1080×1920, 16s, dual-purpose IG+YT) built from the user's own Instagram strategy
doc (7 resistance-to-change binary questions for the "Медитируй УКА" brand) and a bird-mark logo
extracted from a Canva-brand PDF. New engine `lib/reflect.tsx` (`ReflectCard`) — logo settles,
question fades in, two answer options fade in, everything fades back out to the logo-only frame
0 state for a seamless loop. Reused across all 7 shots with content-only diffs.

- **Full QA on short-16 (first use of the new pattern) caught a real bug**: the extracted logo
  PNG had a faint rectangular halo — `sips`'s PDF rasterization anti-aliased the source black
  card's own edge, and a naive luminance→alpha conversion turned that into visible ghosting.
  Fixed with a hard luminance cutoff before the alpha conversion. Confirms the QA-depth rule
  (see `feedback_qa_depth_calibration` memory / short-14 vs short-15 benchmark): new visual
  pattern → full QA earns its cost.
- **The rule needed a refinement, though.** Light QA on short-17..22 (reused template) initially
  planned as "frame 0 + last frame," per the established default — but for THIS template both
  those frames show ONLY the logo (question/options live mid-timeline, faded out by the loop),
  so hook+loop is blind to this template's actual risk: text overflow on longer questions. Added
  one mid-timeline frame for the two longest questions before calling it QA'd. **Light QA should
  target whichever frame(s) actually exercise the content, not literally "first and last."**
- Palette deliberately distinct from the entertainment shorts (deep indigo `#12172a` / warm
  paper-white `#f3ede0` / muted sand-gold `#c9a875`) — calmer register for a reflection/meditation
  brand vs. the hook-driven energetic gold used elsewhere.
- Per the source strategy doc's own plan: Stories-test these 7 themes for engagement before
  investing promotion behind any single one — building the videos doesn't skip that step.

## short-27 · aphorism short form (2026-08-07)

YouTube analytics for the "Медитируй УКА" channel (2026-07-09 → 2026-08-06, outside this repo)
showed single-phrase "remember this when life is heavy" videos at 8-16s far outperforming
everything else (top 4: 1634/1484/1375/1374 views), while the same theme stretched to 32-34s
with an explainer + comment-bait question collapsed to ~700-800. **short-27-aphorism-stars**
tests that exact shape on the short-23..26 engine: new `StarField` scene in `lib/aphorism.tsx`,
one kicker line + one payoff line (both pre-composed at frame 0, series convention), silence,
loop — no explainer paragraph, no question/invite-chip beat. 14s total.
- Voice + wind-soft/chime-magic SFX audition done; awaiting the user's ear (hard audit gate)
  and, if this ships and performs, the plan is to re-cut short-23..26's remaining slate to this
  length instead of the long shape.
